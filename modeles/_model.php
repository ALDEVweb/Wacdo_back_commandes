...
use fraldev\modeles\_field;
use fraldev\modeles\fragment;
    ...
    // Méthodes // 
        ...
        // Getters :
            ...
            function getTarget($field){
                // retourne l'objet pointé par un champ
                $class = '\\fraldev\\classes\\'; 
                $class .= $this->fields[$field]->link();
                $idLink = $this->fields[$field]->get();
                return new $class($idLink);
            }
            ...
            function genForm($nom, $fields, $oeil="noir"){
                ...
                foreach($fields as $field){
                    ...
                    // je récupère le type du champ, son libellé, et sa valeur
                        ...
                        $value = !is_null($this->fields[$field]->get()) ? htmlentities($this->fields[$field]->get()) : "";
                        // j'ouvre une div, je met la balise label
                        $html .= "<div class='mt16 mrlauto small-10'>";
                        $html .= "<label class='txt-center d-block mrlauto small-12 medium-12 large-10 txt-one' for='$nom-$field'>$libelle</label>";
                        if($type == "TEXTAREA"){
                            // si type = TEXTAREA
                            $html .= "<textarea class='d-block mrlauto small-12 medium-12 large-10 mt8' rows='5' name='$libelle' id='$nom-$field'>$value</textarea>";  
                        }else{
                            // si type = password
                            if($type == "PASSWORD"){
                                $nbrMdp++;
                                // je crée le champ pwd avec input, l'oeil et si plrs champs pwd referencement de data-pwd pour les differencier
                                $html .= "<div class='pwd-field flex j-between mt8 small-12 medium-12 large-10 mrlauto'>";
                ...
                if($nbrMdp === 1) $html .= "<p id='mdp-oubli' class='fs12 txt-center txt-two mt16'>Mot de passe oublié ?</p>";
                if($nbrMdp > 1){
